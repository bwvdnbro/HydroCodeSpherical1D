...
#define SEDOV_NCELL_INJECTION 1
...
#define initialize(cells, ncell)                                               \
  _Pragma("omp parallel for") for (unsigned int i = 1; i < ncell + 1; ++i) {   \
    if (i < SEDOV_NCELL_INJECTION + 1) {                                       \
      cells[i]._P =                                                            \
          (GAMMA - 1.) * (0.5 / SEDOV_NCELL_INJECTION) / cells[i]._V;          \
    } else {                                                                   \
      cells[i]._P = 1.e-6;                                                     \
    }                                                                          \
    cells[i]._rho = 1.;                                                        \
...
#define initialize(cells, ncell)                                               \
  _Pragma("omp parallel for") for (unsigned int i = 1; i < ncell + 1; ++i) {   \
    ...
    if (i < SEDOV_NCELL_INJECTION + 1) {                                       \
      ...
      cells[i]._P =                                                            \
          (GAMMA - 1.) *                                                       \
          (0.25 * 5.e43 / SEDOV_NCELL_INJECTION / UNIT_ENERGY_IN_SI) / Vs;     \
    } else {                                                                   \
      /*cells[i]._P = 1.e-6;*/                                                 \
      cells[i]._P = 1.e4 * cells[i]._rho * BOLTZMANN_K_IN_SI /                 \
                    HYDROGEN_MASS_IN_SI / UNIT_VELOCITY_IN_SI /                \
                    UNIT_VELOCITY_IN_SI;                                       \
    }                                                                          \
    cells[i]._u = 0.;                                                          \
    cells[i]._a = 0.;                                                          \
